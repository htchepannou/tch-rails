    ...
    implements Serializable
{
    ...
    public Map toProperties ()
    {
        ...
        for ( Method m: getClass ().getMethods () )
        {
            ...
            String property = name.length () > plen
                ? name.substring (plen, plen+1).toLowerCase () + name.substring (plen+1)
                : name.substring (plen, plen+1).toLowerCase ();
            try
            {
    ...
    protected ActiveRecord toActiveRecord (Map properties, String name, Class type)
    {
        ...
        return id > 0
            ? ( ActiveRecord ) getEntityManager ().find (type, id)
            : null;
    }
