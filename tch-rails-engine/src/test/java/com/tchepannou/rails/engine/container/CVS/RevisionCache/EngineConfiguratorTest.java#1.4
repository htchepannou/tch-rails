...
public class EngineConfiguratorTest extends TestCase
{
        ...
        throws Exception
    {
        ...
        assertEquals ("actionInterceptors", 6, engine.getActionInterceptors ().size ());

        assertEquals ("jobsInteceptors", 4, engine.getJobInterceptors ().size ());

        assertEquals ("mailInteceptors", 2, engine.getMailInterceptors ().size ());

        assertEquals ("messsageInteceptors", 4, engine.getMessageInterceptors ().size ());
